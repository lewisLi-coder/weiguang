...
test('redisHistoryStore reads messages for a clientId', async () => {
  ...
  assert.equal(fake.calls[0].url, 'https://redis.example.com/get/chat%3Ahistory%3Aclient-1');
...
test('redisHistoryStore replaces normalized messages for a clientId', async () => {
  ...
  assert.equal(fake.calls[0].url, 'https://redis.example.com/set/chat%3Ahistory%3Aclient-1');
  assert.deepEqual(fake.calls[0].body, [JSON.stringify(saved)]);
});
...
test('redisHistoryStore appends a message for a clientId', async () => {
  ...
  assert.equal(fake.calls[1].url, 'https://redis.example.com/set/chat%3Ahistory%3Aclient-1');
  assert.equal(JSON.parse(fake.calls[1].body[0]).length, 2);
});
